function stats = allfixations_retinal(subj, ngrps, grpsz, blkrng)
%function stats = allfixations_retinal(subj, ngrps, grpsz[, blkrng])
% Compute density map and the associated statistics of object's retinal 
% position for the selected saccades (whichsaccades) after each object 
% movement (whichobjectperiods) within the blocks in blkrng.
...
% 4/2011 bst wrote it
% 6/2011 bst add BCEA at 68%, following Bellmann et al.2004
% 9/2011 bst change the calculation of stats to use mean xy / duration of
% and added a minimum latency requirement after object movement


% parameters
bceathd = .68;
minlatency = 0.1;
minduration = 0.1; % discard any fixations with durations < 100ms
kdetrim = 0.1;
...
for i = 1:ngrps
    ...
    b0 = (i-1)*step+1;
    blks = b0:b0+grpsz-1;
    fxa = eymobj.fxa(trials,blkrng(blks),phases);
    obj = eymobj.obj(trials,blkrng(blks),phases);
    
    fxa = selectwrtobj(fxa,obj,whichsaccades,whichobjectperiods,minlatency);
   
    % use mean/duration of each fixation for the stats (post VSS 2011)
    mx = [];
    my = [];
    dur = [];
    for k=1:numel(fxa)
        [~,~,jj]=unique(fxa(k).rawgrp);
        mx = [mx fxa(k).meanx(jj)];
        my = [my fxa(k).meany(jj)];
        dur = [dur fxa(k).duration(jj)];
    end
    mx = mx(dur>minduration);
    my = my(dur>minduration);
    xy2 = mx';
    if isempty(xy2)
        continue
    end
    ...
    xy2(:,2) = my';
    xy = -xy2; % object's retinal position
...
%     end
%     xy(:,2) = [fxa.rawy]';
%     xy = -xy; % object's retinal position

%     [~,density,X,Y] = kde2d(xy);
    %[bandwidth,density,X,Y] = kde2d(xy,64,[-20 -20], [20 20]);
    ...
    % principal axes.  Line length == 1 SD (previously: 1.96 SD for a 95% CI)
    ...
    % save a figure
    fname = sprintf('%s_fixa_%d_%d_%d_%d',subj,ngrps,grpsz,blkrng(1),h);
    print('-depsc','-tiff','-r300',fname);
end
fname = sprintf('%s_fixa_%d_%d_%d.mat',subj,ngrps,grpsz,blkrng(1));
if exist('stats','var')
    save(fname,'stats');
end
